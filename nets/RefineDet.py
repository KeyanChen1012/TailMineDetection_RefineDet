...
class RefineSSD(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.is_refine:
            for (arm_x, arm_l, arm_c) in zip(arm_sources, self.arm_loc, self.arm_conf):
                # self.arm_loc nsamples*12*featuremapSize
                arm_loc_list.append(arm_l(arm_x).permute(0, 2, 3, 1).contiguous())
                arm_conf_list.append(arm_c(arm_x).permute(0, 2, 3, 1).contiguous())
        ...
        for (input_x, trans) in zip(arm_sources[0:3], self.trans_layers):
